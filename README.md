...
- **新增 Web 资产自动扫描**：任务发现的 Web 按项目意图触发——项目下勾选了漏洞扫描的任务才执行漏洞扫描；资产导入 / 空间测绘导入的 Web 由实时扫描器异步补扫（标准语义，`scan.web_autoscan` 可关，白名单跳过）
...
- **WIH JS 敏感信息检测**：随弱点检测阶段扫描站点首页与引用 JS，正则匹配云 AK/SK、JWT、密码、机器人 Webhook 等泄露，命中归入弱点管理（默认规则集移植自 [WIHscan](https://github.com/ifacker/WIHscan)，MIT；规则在弱点管理页管理，支持单 URL 即时检测）
- **敏感字词库订阅**：订阅任意 GitHub 敏感词仓库或自定义 raw 源（默认预设 [konsheng/Sensitive-lexicon](https://github.com/konsheng/Sensitive-lexicon)，MIT，可一键恢复）——仓库地址自动归一、文件清单自动发现或手工添加、直连失败镜像重试、每日定时自动更新；订阅词与手动词库合并参与页面敏感字检测（UTF-8/GBK 自动归一），入库词条提供管理弹窗（分页搜索/手工增补/删除记入排除清单）

### AI 研判

- 对接 OpenAI 兼容接口（DeepSeek / Qwen / GPT / Ollama 等均可），界面配置 base_url / api_key / model
- 一键获取可用模型列表；单条研判或"AI 全部研判"批量处理
- 扫描完成后可自动对新检出漏洞做误报/实报研判，给出置信度与理由；开启「AI 自动研判」后**实时研判**——新增漏洞入库即异步分析打标（含任务发现、资产导入、POC 监控等全部来源），队列限流不阻塞扫描
...
| 仪表盘（5 秒实时刷新：漏洞风险 / 弱点分布 / 最近检出） | 资产管理（IP / 域名 / 端口 / Web 资产） |
|:---:|:---:|
| ![仪表盘](docs/screenshots/dashboard.png) | ![资产管理](docs/screenshots/assets.png) |

| 漏洞风险（严重度徽章 / 发现时间 / 标记 / AI 研判） | 弱点管理（暗链 / 坏链 / 敏感字 / WIH + 标记筛选） |
|:---:|:---:|
| ![漏洞风险](docs/screenshots/vulns.png) | ![弱点管理](docs/screenshots/weakness.png) |

| 扫描任务（执行内容复选框 + 条件子项） | 变化监控（关键词 / 类型检索 + 分页） |
|:---:|:---:|
| ![扫描任务](docs/screenshots/tasks.png) | ![变化监控](docs/screenshots/changes.png) |
...
| [konsheng/Sensitive-lexicon](https://github.com/konsheng/Sensitive-lexicon) | MIT | **敏感字词库订阅源**：分类敏感词库（Vocabulary 目录 17 个文件）按需勾选拉取，自动归一编码、去重与脏行过滤 |
