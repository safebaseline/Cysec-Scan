...
- 子域名枚举：域名目标自动爆破子域名 + 证书透明度被动收集（[crt.name](https://crt.name)，不依赖字典、可发现历史子域），解析出的新 IP 纳入后续扫描；任务域名与收集到的子域名按 https/http 探测为 Web 资产（覆盖 vhost / CDN 同 IP 多站点场景）
...
- 全局出站代理：http / socks5（含用户名密码认证），所有扫描流量统一出口，配置写回 config.yaml
