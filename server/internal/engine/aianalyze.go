...
// aiAnalyzeJob 实时研判任务（kind=vuln 漏洞 / weakness 弱点，携带研判上下文）
...
// SubmitWeaknessAIAnalyze 提交弱点的实时研判（标记写回弱点表）
func (e *Engine) SubmitWeaknessAIAnalyze(id int64, ctx ai.VulnContext) {
	if e.aiQueue == nil || id <= 0 {
		return
	}
	select {
	case e.aiQueue <- aiAnalyzeJob{id: id, ctx: ctx, kind: "weakness"}:
	default:
		log.Printf("[AI] 研判队列已满（%d），跳过弱点 %s", aiAnalyzeQueueCap, ctx.VulnID)
	}
}
