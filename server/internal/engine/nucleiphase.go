...
func (e *Engine) runNucleiBatchJobs(jobs []autoScanJob) {
	...
	// Web 资产字段映射（按项目缓存，IP/端口/域名回填）；白名单在消费侧统一收口——
	// 各生产路径（任务内联/实时扫描/独立阶段补扫）的白名单资产一律不进 nuclei 引擎
	// （AssetWeb 的 IP/Domain 可能为空，whitelistHit 从 URL host 兜底）
	webCache := map[int64][]model.AssetWeb{}
	websOf := func(projectID int64) []model.AssetWeb {
		if _, ok := webCache[projectID]; !ok {
			webCache[projectID] = e.store.WebsOfProject(projectID)
		}
		return webCache[projectID]
	}
	webOf := func(j autoScanJob) *model.AssetWeb {
		for i := range websOf(j.projectID) {
	...
	for _, j := range jobs {
		...
		if w := webOf(j); w != nil && e.whitelistHit(w.IP, w.Domain, w.URL) {
			continue
		}
	...
	matched := 0
	suppressed := 0
	baselines := map[string]softBaseline{}
	...
	for _, f := range findings {
		...
		// soft-404 误报抑制：命中响应与该站点"任意路径基准响应"指纹一致（SPA/通配路由
		// 站点对不存在的路径也返回同一 200 页面）说明响应无路径区分度，判定为误报
		if _, ok := baselines[j.url]; !ok {
			baselines[j.url] = baselineOf(j.url, maxInt(task.TimeoutSec, 5))
		}
		if isSoft404Response(resp, baselines[j.url]) {
			suppressed++
			log.Printf("[nuclei] soft-404 误报抑制：%s %s（命中响应与站点基准响应一致）", f.TemplateID, orDefaultStr(f.MatchedAt, f.URL))
			continue
		}
		if strings.TrimSpace(req) == "" {
	...
	if suppressed > 0 {
		e.taskLog(0, "info", fmt.Sprintf("nuclei 官方引擎：抑制 soft-404 误报 %d 条（站点对任意路径返回同一响应，命中结果不具备路径区分度）", suppressed))
	}
	if matched == 0 {
...
func (e *Engine) ScanAssetsWithNucleiRules(ruleIDs []string, timeoutSec int) int {
	...
	for _, f := range findings {
		...
		// 报文兜底：事件缺报文时对命中地址补抓真实请求/响应（失败再走重建）
		req, resp := f.Request, f.Response
		if strings.TrimSpace(req) == "" || strings.TrimSpace(resp) == "" {
		...
		// soft-404 误报抑制（同实时扫描口径）
		if _, ok := baselines[w.URL]; !ok {
			baselines[w.URL] = baselineOf(w.URL, timeoutSec)
		}
		if isSoft404Response(resp, baselines[w.URL]) {
			suppressed++
			log.Printf("[nuclei] soft-404 误报抑制（新增规则扫描）：%s %s", f.TemplateID, orDefaultStr(f.MatchedAt, f.URL))
			continue
		}
		if strings.TrimSpace(req) == "" {
	...
	log.Printf("[nuclei] 新增规则扫描：%d 资产 × %d 规则，检出 %d，抑制 soft-404 误报 %d", len(urls), len(ruleIDs), matched, suppressed)
	return matched
}
...
// stop 关闭（批次结束/取消）时释放全部监听 goroutine——任务的 cancel 通道在正常结束时
// 不会关闭，无此释放机制每批会泄漏 N 个永久阻塞的 goroutine。
func waitAny(chans []chan struct{}, stop <-chan struct{}) chan struct{} {
