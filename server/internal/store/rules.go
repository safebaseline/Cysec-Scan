...
func (s *Store) ListVulnRules(where string, args []any, order string, limit, offset int) ([]map[string]any, int, error) {
	...
	var total int
	s.db.QueryRow(`SELECT COUNT(*) FROM vuln_rules`+map[bool]string{true: " WHERE " + where, false: ""}[where != ""], args...).Scan(&total)
	if order == "" {
		order = "id DESC"
	}
...
// NewWebsSince 查询任务在指定时间之后新增（change=add）的 Web 资产 URL（供周期任务对新资产默认漏洞扫描）
func (s *Store) NewWebsSince(taskID int64, since time.Time) []string {
	rows, err := s.db.Query(`SELECT asset FROM asset_changes
		WHERE task_id=? AND asset_type='web' AND change='add' AND created_at >= datetime(?, 'localtime')`,
		taskID, since.Add(-2*time.Second).Format("2006-01-02 15:04:05"))
	if err != nil {
		return nil
	}
	defer rows.Close()
