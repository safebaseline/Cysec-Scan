...
func (api *API) safePocPath(rel string) (string, error) {
	...
	if strings.Contains("/"+rel+"/", "/../") { // 按路径段判断（允许文件名含 ".."，穿越由段边界拦截）
...
func (api *API) importPocFiles(c *gin.Context) {
	...
	res, newRules, err := vulnrule.ImportDirCollect(full, api.store.UpsertVulnRule)
	if err != nil {
		c.JSON(500, gin.H{"error": err.Error()})
		return
	}
	// 与规则目录导入/POC 目录监控同口径：新增可执行规则 → 对存量 Web 资产补扫
	...
	api.store.SystemLog(model.SystemLog{Username: c.GetString("username"), Action: "poc_import", Object: req.Path, ClientIP: c.ClientIP(),
		Result: fmt.Sprintf("new=%d update=%d 触发存量资产补扫规则=%d", res.Imported, res.Updated, len(usable))})
	c.JSON(200, res)
}
