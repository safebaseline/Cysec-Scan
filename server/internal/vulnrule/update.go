...
// 拦截以 "-" 开头（会被 git 解析为选项，如 --upload-pack）、含空白/控制字符
// （可借换行等污染 git 配置或终端）或伪协议（如 ext::/ssh://，git 会按传输协议执行命令）的值。
...
// repoDirName 源 URL → 模板根目录下的克隆目录名（保持既有命名兼容：
// owner/repo 两段式；仅清洗分隔符与 ".." 段，防止克隆路径逃出模板根目录）
func repoDirName(url string) string {
	sanitize := func(s string) string {
		s = strings.ToLower(strings.NewReplacer("/", "_", "\\", "_", ":", "_").Replace(s))
		if s == "" || s == "." || s == ".." {
			return "_"
		}
		return s
	...
	if len(parts) >= 2 {
		return filepath.Join(sanitize(parts[len(parts)-2]), sanitize(strings.TrimSuffix(parts[len(parts)-1], ".git")))
	}
	return sanitize(strings.NewReplacer(".", "_").Replace(strings.TrimRight(strings.TrimSpace(url), "/")))
}
...
// gitCommand 构造模板源更新用的 git 命令：始终直连，不走全局代理
// （全局代理仅服务扫描流量；GitHub 可达性由镜像前缀/每日更新的镜像加速解决）
