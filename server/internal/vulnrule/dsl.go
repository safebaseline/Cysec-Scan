...
// dslMaxDepth 递归深度上限：恶意模板可用数十万层括号/感叹号把递归下降解析器
// 打出栈溢出（recover 接不住），超限直接判表达式错误
const dslMaxDepth = 500

// EvalDSL 求值 nuclei DSL 表达式，返回 bool 与错误。
// panic 兜底：单个匹配器异常不拖垮整个扫描进程（规则来自第三方模板源）。
func EvalDSL(expr string, ctx DSLContext) (ok bool, err error) {
	defer func() {
		if r := recover(); r != nil {
			ok, err = false, fmt.Errorf("dsl panic: %v", r)
		}
	}()
...
func (p *dslParser) parseExpr() (any, error) {
	if p.depth >= dslMaxDepth {
		return nil, fmt.Errorf("表达式嵌套过深（>%d 层，疑似恶意模板）", dslMaxDepth)
	}
	p.depth++
	defer func() { p.depth-- }()
	return p.parseOr()
