...
// panic 兜底：规则来自第三方模板源，单条规则异常不允许拖垮扫描进程。
func Run(rule *Rule, target string, timeoutSec int) (res RunResult) {
	defer func() {
		if r := recover(); r != nil {
			res = RunResult{Err: fmt.Sprintf("规则执行 panic: %v", r)}
		}
	}()
	var exec ExecRule
	if rule.Parsed != "" {
	...
	// 基线指纹与 WAF 探测按目标缓存（10 分钟）：逐条执行数千条规则时避免对同一目标
	// 放大 3N 倍额外探测请求（随机路径基线 ×2 + 根路径 ×1），降低被 WAF 封禁的概率
	baselineHash, wafBlocked := probeTarget(base, timeoutSec)
...
// probeTargetCache 目标探测缓存：基线指纹（CDN/WAF/SPA 随机路径基线）与 WAF 拦截
// 判定按 base URL 缓存 10 分钟——Run 每条规则都会用到，逐规则重复探测会放大 3N 倍请求
var probeTargetCache sync.Map // string -> *probeResult
...
// probeTarget 返回目标的（基线指纹, 是否疑似被 WAF 拦截），带 TTL 缓存
func probeTarget(base string, timeoutSec int) (string, bool) {
	if v, ok := probeTargetCache.Load(base); ok {
		if p := v.(*probeResult); time.Since(p.at) < 10*time.Minute {
			return p.baseline, p.waf
		}
	}
	b, _ := baselineFingerprint(base, timeoutSec)
	w := detectWAF(base, timeoutSec)
	probeTargetCache.Store(base, &probeResult{baseline: b, waf: w, at: time.Now()})
	return b, w
}
...
func isSafeMethod(steps []ExecStep) bool {
	for _, s := range steps {
		switch s.Method {
		case "GET", "POST", "HEAD", "OPTIONS": // 非破坏性：PUT/DELETE/PATCH 可改写/删除目标资源，拒绝执行
		default:
			return false
		}
	}
...
func dumpRequest(method, url string, req *http.Request, body string) string {
	...
	b.WriteString("\r\n")
	if body != "" {
		if r := []rune(body); len(r) > 64*1024 { // 请求体截断入库，避免 MB 级报文膨胀
			b.WriteString(string(r[:64*1024]) + "\n[已截断]")
		} else {
			b.WriteString(body)
		}
	}
...
func dumpResponse(resp *http.Response, body []byte) string {
	...
	b.WriteString("\r\n")
	if len(body) > 64*1024 { // 响应体截断入库，避免单条命中写入 MB 级报文
		b.Write(body[:64*1024])
		b.WriteString("\n[已截断，原始 " + strconv.Itoa(len(body)) + " 字节]")
	} else {
		b.Write(body)
	}
	return b.String()
...
func matchGroup(g ExecGroup, status int, body, headerText string, elapsedMs int64, dslMethod string) bool {
	...
	case "prefix":
		// and=全部词均为前缀，or=任一词为前缀（修复：原实现 and 全命中时 ok 恒 false）
		if g.Condition == "and" {
			ok = true
...
func headersText(resp *http.Response) string {
	...
	for k, vs := range resp.Header {
		...
		for _, v := range vs {
			b.WriteString(lk + ": " + v + "\r\n") // 与 extractHeader 的 CRLF 切分一致，否则整个头块被当一行导致 Content-Type/Location 取值错乱
		}
	}
	return b.String()
}
