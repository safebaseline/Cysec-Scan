...
func TestPerProviderInterval(t *testing.T) {
	def := providerMinIntervalMs["fofa"]
	if got := effectiveInterval("fofa", Config{FOFAIntervalMs: 5000, IntervalMs: 100}); got != 5000 {
		t.Fatalf("provider-specific should win: %d", got)
	}
	if got := effectiveInterval("fofa", Config{FOFAIntervalMs: 300}); got != def {
		t.Fatalf("below builtin floor should clamp to default: %d", got)
	}
	if got := effectiveInterval("quake", Config{IntervalMs: 2000, QuakeIntervalMs: 0}); got != 2000 {
		t.Fatalf("global fallback: %d", got)
	}
	if got := effectiveInterval("shodan", Config{}); got != providerMinIntervalMs["shodan"] {
		t.Fatalf("builtin default: %d", got)
	}
}
