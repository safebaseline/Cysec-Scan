...
export const api = {
  ...
  getFPAssets: (pid: number) => request(`/api/fp-assets?project_id=${pid}`),
  setFPAssets: (pid: number, items: string[]) => request(`/api/fp-assets?project_id=${pid}`, { method: 'PUT', body: JSON.stringify({ items }) }),
  ...
  ctCollect: (domain: string) => request('/api/subdomains/ct?domain=' + encodeURIComponent(domain)),
  ...
  getProxyHealth: () => request('/api/system/proxy-health'),
  systemLogs: (page = 1, size = 20, q = '', type = '') =>
    request(`/api/system/logs?limit=${size}&offset=${(page - 1) * size}` + (q ? `&q=${encodeURIComponent(q)}` : '') + (type ? `&type=${type}` : '')),
  systemLogsClear: () => request('/api/system/logs', { method: 'DELETE' }),
  setProxyHealth: (payload: { interval_sec?: number; enabled?: boolean; target?: string }) => request('/api/system/proxy-health', { method: 'PUT', body: JSON.stringify(payload) }),
  setProxy: (proxy: any) =>
  ...
  getLeakPaths: () => request('/api/system/leak-paths'),
  weaknesses: (pid: number, page = 1, size = 20, q = '', type = '', mark = '') =>
    request(`/api/weaknesses?project_id=${pid}&limit=${size}&offset=${(page - 1) * size}` +
      (q ? `&q=${encodeURIComponent(q)}` : '') + (type ? `&type=${type}` : '') + (mark ? `&mark=${mark}` : '')),
  weaknessDelete: (id: number) => request(`/api/weaknesses/${id}`, { method: 'DELETE' }),
  weaknessMark: (id: number, mark: string) => request(`/api/weaknesses/${id}/mark`, { method: 'PUT', body: JSON.stringify({ mark }) }),
  weaknessAI: (id: number) => request(`/api/weaknesses/${id}/ai-analyze`, { method: 'POST' }),
  weaknessAIAll: (pid: number) => request(`/api/weaknesses/ai-analyze-all?project_id=${pid}`, { method: 'POST' }),
  weaknessClear: (pid: number, type = '') => request(`/api/weaknesses?project_id=${pid}` + (type ? `&type=${type}` : ''), { method: 'DELETE' }),
  getWeaknessSettings: () => request('/api/weakness/settings'),
  getDomainConsistency: () => request('/api/system/domain-consistency'),
  setDomainConsistency: (enabled: boolean) => request('/api/system/domain-consistency', { method: 'PUT', body: JSON.stringify({ enabled }) }),
  domainCleanup: (roots: string[]) => request('/api/system/domain-consistency/cleanup', { method: 'POST', body: JSON.stringify({ roots }) }),
  setWeaknessSettings: (cfg: any) => request('/api/weakness/settings', { method: 'PUT', body: JSON.stringify(cfg) }),
  setSensSub: (sub: any) => request('/api/weakness/senssub', { method: 'PUT', body: JSON.stringify(sub) }),
  // 立即更新 = 先保存当前界面配置（含刚勾选未保存的文件）再拉取
  updateSensSub: (sub: any) => request('/api/weakness/senssub', { method: 'PUT', body: JSON.stringify({ ...sub, update: true }) }),
  discoverSensSub: (base: string) => request('/api/weakness/senssub/discover', { method: 'POST', body: JSON.stringify({ base }) }),
  sensSubWords: (q: string, offset = 0) => request('/api/weakness/senssub/words?q=' + encodeURIComponent(q) + '&offset=' + offset + '&limit=100'),
  sensSubWordsAdd: (words: string[]) => request('/api/weakness/senssub/words', { method: 'POST', body: JSON.stringify({ words }) }),
  sensSubWordsDel: (words: string[]) => request('/api/weakness/senssub/words', { method: 'DELETE', body: JSON.stringify({ words }) }),
  sensSubWordsClear: () => request('/api/weakness/senssub/words/clear', { method: 'POST' }),
  sensSubExcludes: (q = '') => request('/api/weakness/senssub/excludes?q=' + encodeURIComponent(q)),
  sensSubExcludesRestore: (words: string[]) => request('/api/weakness/senssub/excludes/restore', { method: 'POST', body: JSON.stringify({ words }) }),
  setLeakPaths: (items: any[]) => request('/api/system/leak-paths', { method: 'PUT', body: JSON.stringify({ items }) }),
